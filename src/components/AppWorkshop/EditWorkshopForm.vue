...
onBeforeMount(() => {
	newWorkshop.value.renderImg = [router.currentRoute.value.params.image];
	if(userStore.user.isAdmin){
		newWorkshop.value.renderImgLogo = [router.currentRoute.value.params.imageLogo];
		newWorkshop.value.asociationName = router.currentRoute.value.params.asociationName
		newWorkshop.value.email = router.currentRoute.value.params.email
		newWorkshop.value.imageLogo = router.currentRoute.value.params.imageLogo
		if(router.currentRoute.value.params.uid){
			newWorkshop.value.userId = router.currentRoute.value.params.uid
		}
	}
})
onMounted(() => {
...
<template>
	<main class="content">
		<h1 class="form-title">Editar Taller: <br/> {{newWorkshop.title}}</h1>
